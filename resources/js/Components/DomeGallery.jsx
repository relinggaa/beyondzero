...
const DEFAULT_IMAGES = [
  ...
  {
    src: 'https://images.unsplash.com/photo-1494790108755-2616b612b786?q=80&w=774&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D',
    alt: 'Sari Dewi',
    name: 'Sari Dewi',
    review: 'Platform yang luar biasa! Saya bisa konsultasi kapan saja tanpa harus keluar rumah. Sangat membantu untuk kesehatan mental saya.',
    rating: 5
  },
  {
...
function buildItems(pool, seg) {
  ...
  const normalizedImages = pool.map(image => {
    if (typeof image === 'string') {
      return { src: image, alt: '', name: '', review: '', rating: 0 };
    }
    return { 
      src: image.src || '', 
      alt: image.alt || '', 
  ...
  return coords.map((c, i) => ({
    ...
    review: usedImages[i].review,
    rating: usedImages[i].rating
  }));
}
...
export default function DomeGallery({
  ...
  const startAutoRotate = useCallback(() => {
    if (!autoRotate || autoRotatePausedRef.current) return;
    
    const step = () => {
      if (autoRotatePausedRef.current || draggingRef.current || focusedElRef.current) {
    ...
    onDrag: ({ event, last, velocity = [0, 0], direction = [0, 0], movement }) => {
      ...
      if (last) {
        ...
        if (Math.abs(vx) > 0.005 || Math.abs(vy) > 0.005) {
          ...
          // Resume auto rotation after drag ends
          setTimeout(() => {
            ...
            requestAnimationFrame(() => {
              ...
              setTimeout(() => {
                ...
                if (!draggingRef.current && rootRef.current?.getAttribute('data-enlarging') !== 'true') {
                  document.body.classList.remove('dg-scroll-lock');
                  // Resume auto rotation after image closes
                  setTimeout(() => {
                    if (!draggingRef.current && !focusedElRef.current) {
                      startAutoRotate();
                    }
  ...
  const openItemFromElement = useCallback(el => {
    ...
    const userReview = parent.dataset.review || '';
    const userRating = parent.dataset.rating || 0;
    ...
    reviewOverlay.innerHTML = `
      ...
      <p style="margin: 0; font-size: 13px; line-height: 1.4; opacity: 0.9;">${userReview}</p>
    `;
    ...
    setTimeout(() => {
      ...
      // Animate review overlay
      setTimeout(() => {
        reviewOverlay.style.transform = 'translateY(0)';
      }, 200);
          ...
          <div ref={sphereRef} className="sphere">
            {items.map((it, i) => (
              <div
                ...
                data-src={it.src}
                data-name={it.name}
                data-review={it.review}
                data-rating={it.rating}
                data-offset-x={it.x}
                data-offset-y={it.y}
                ...
                <div
                  ...
                  role="button"
                  tabIndex={0}
                  aria-label={it.name || it.alt || 'View user review'}
                  onClick={onTileClick}
